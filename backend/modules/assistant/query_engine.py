...
def answer_investigation_query(query: str, active_dataset: Dict[str, Any], impact_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ---------------------------------------------------------
    # 2. INFRASTRUCTURE DEPENDENCY CASCADE
    # ---------------------------------------------------------
    elif any(phrase in q for phrase in ["cascade", "dependency", "consequence", "domino", "knock-on"]):
        ...
        text = (
            f"**Explainable Infrastructure Dependency Cascade ({location}):**\n\n"
    ...
    # ---------------------------------------------------------
    # 3. SPECIFIC HOSPITALS & EMERGENCY HEALTHCARE FACILITIES
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 4. SPECIFIC SCHOOLS, ACADEMIES & CIVIC SHELTERS
    # ---------------------------------------------------------
    elif any(phrase in q for phrase in [
    ...
    # ---------------------------------------------------------
    # 5. SATELLITE SENSORS, RESOLUTION & DETECTION METHODOLOGY
    # ---------------------------------------------------------
    elif any(phrase in q for phrase in [
    ...
    # ---------------------------------------------------------
    # 6. TACTICAL RECOMMENDATIONS & FIELD RESPONSE PROTOCOL
    # ---------------------------------------------------------
    elif any(phrase in q for phrase in [
    ...
    # ---------------------------------------------------------
    # 7. PERSISTENT GEOGRAPHIC HOTSPOTS & MULTI-YEAR TRENDS
    # ---------------------------------------------------------
    elif any(phrase in q for phrase in [
    ...
    # ---------------------------------------------------------
    # 8. HIGH PRIORITY / URGENT ATTENTION AREAS / WHY HIGH PRIORITY
    # ---------------------------------------------------------
    elif any(phrase in q for phrase in [
        ...
        bullets = priority_info.get("explanation_bullets", [])
        bullet_text = "\n".join([f"  {b}" for b in bullets]) if bullets else f"  • {priority_info.get('rationale', 'Critical infrastructure exposure')}"
        narrative = cascade.get("dependency_narrative", "")
        ...
        text = (
            ...
            f"{bullet_text}\n\n"
            f"**Infrastructure Dependency Detail:**\n"
            f"{narrative if narrative else 'Ground transport access to primary hospitals and evacuation routes compromised.'}\n\n"
        ...
        citations = ["EarthLens Prioritization Engine", "EarthLens Dependency Engine", "Spatial Proximity Matrix", f"{location} AOI"]
